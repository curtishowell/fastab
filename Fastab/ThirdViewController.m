...
//  Fastab
//
//  Created by studentuser on 5/31/13.
...
@property (weak, nonatomic) IBOutlet UITableView *cancelledOrderTable;
...
- (void)viewDidLoad
{
    ...
    self.cancelledOrderTable.backgroundColor = bgColor;
    
    self.cancelledOrderTable.dataSource = self;
    self.cancelledOrderTable.delegate = self;
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.cancelledOrderTable) {
        
        int width = self.cancelledOrderTable.frame.size.width;
...
{
    if(indexPath.row == ((NSIndexPath*)[[self.cancelledOrderTable indexPathsForVisibleRows] lastObject]).row){
